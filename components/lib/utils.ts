...
const colorNames: ColorName[] = ['blue','purple','green','yellow','orange','red','indigo','pink','cyan','gray'];
function normalizeColor(color: string): ColorName {
  return (colorNames as string[]).includes(color) ? (color as ColorName) : 'blue';
}

export function colorToBg(color: ColorName | string, shade: 50 | 100 | 200 | 300 | 400 | 500 | 600 | 700 = 500) {
  const c = normalizeColor(color as string);
...
export function colorToText(color: ColorName | string, shade: 600 | 700 = 600) {
  const c = normalizeColor(color as string);
  const map: Record<ColorName, Record<number, string>> = {
...
export function colorToBorder(color: ColorName | string, shade: 200 | 300 | 400 = 300) {
  const c = normalizeColor(color as string);
  const map: Record<ColorName, Record<number, string>> = {
